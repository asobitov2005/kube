...
helm upgrade --install rabbitmq charts/rabbitmq-ha \
  -n messaging --create-namespace --wait
```

Production uchun:

```bash
helm upgrade --install rabbitmq charts/rabbitmq-ha -n messaging \
  -f charts/rabbitmq-ha/values.yaml \
  -f charts/rabbitmq-ha/values-production.yaml --wait
```

Credential va tekshiruv:

```bash
kubectl get rabbitmqcluster,pod,pvc -n messaging
kubectl get secret rabbitmq-default-user -n messaging -o jsonpath='{.data.username}' | base64 -d; echo
kubectl get secret rabbitmq-default-user -n messaging -o jsonpath='{.data.password}' | base64 -d; echo
kubectl port-forward -n messaging service/rabbitmq 15672:15672
```

AMQP manzil: `rabbitmq.messaging.svc.cluster.local:5672`. Management UI: `http://localhost:15672`.
